...
-- questao 12
SELECT DISTINCT F.ID, F.Nome AS 'Fornecedor', P.Nome AS 'NomeDoProduto'
FROM Fornecedores F
INNER JOIN Produtos P ON F.ID = P.Fornecedor
INNER JOIN CategoriasDeProdutos C ON P.Categoria = C.ID
WHERE C.NomeDaCategoria = 'Padaria';
